...
IWSR\,=\,98.3\,\% (LLM-assisted, GPT-4o-mini), with perfect Schema
Compliance (SCR\,=\,100\,\%) and Semantic Validity (SVR\,=\,100\,\%)
across all evaluated configurations and a Stability Score of SS\,=\,0.964.
A complete multi-provider comparison across GPT-4o-mini, Claude Haiku\,4.5, and
Gemini\,2.5\,Flash (59\,/\,60 entries) shows that the pluggable multi-LLM
dispatcher achieves SCR\,$\geq$\,98.3\,\% and SVR\,$\geq$\,98.3\,\% across all
providers, confirming that the schema-grounding mechanism is substantially
provider-agnostic.
The Feedback Engine corrects payload-level validation errors but cannot recover
from Phase\,1 service misclassification; all residual IWSR failures are
service-routing errors on deliberately ambiguous intents.
The platform and evaluation dataset are publicly released as open-source
artifacts providing a reproducible baseline for intent-based orchestration
research.
\end{abstract}
...
\emph{persistence tier} backed by MongoDB (Fig.~\ref{fig:arch}).
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{nof_fig1_arch.png}
  \caption{System architecture overview. Four logical tiers — interface,
  control-plane, provider, and persistence — are deployed as a Docker Compose
  stack. The CAPIF client follows the 3GPP CAPIF-2e reference point for
  invoker onboarding and mTLS-secured discovery; CAMARA providers publish
  through the CAPIF-4 reference point. The three-layer validator (L1--L3)
  intercepts all LLM-generated payloads before any provider call is issued.}
  \label{fig:arch}
\end{figure*}
...
Gemini\,2.5 Flash\textsuperscript{$\dagger$} &  93.3\,\% &  98.3\,\% &  98.3\,\% & 5.0\,\% & 0.884\textsuperscript{$\dagger$} \\
...
\textsuperscript{$\dagger$}Gemini\,2.5\,Flash evaluation completed 59\,/\,60 entries
(1 entry failed with a JSON-format planning error unrelated to model capability).
SS\,=\,0.884 is based on a single evaluation run; $k=5$ stability runs were not performed
for Gemini due to API quota constraints.
GPT-4o-mini results are complete ($k=5$ stability runs, $n=60$ entries).
\end{table}

A key observation is that SCR and SVR remain above 98\,\% across all three
LLM providers, confirming that the schema-grounding mechanism is
substantially provider-agnostic: the three-layer validator intercepts
payload errors before execution regardless of which language model
generated the content.
The minor SCR\,=\,SVR\,=\,98.3\,\% observed for Gemini (vs.\ 100\,\% for
GPT-4o-mini and Claude Haiku) stems from a single planning-level JSON
format error, not from payload schema violations produced by Phase\,2.

GPT-4o-mini achieves the highest IWSR of 98.3\,\% (59/60), with a
hallucination rate of only HR\,=\,1.7\,\% --- outperforming both Claude
Haiku\,4.5 and Gemini\,2.5\,Flash.
Its stability score of SS\,=\,0.964 also surpasses Haiku (SS\,=\,0.892)
and Gemini (SS\,=\,0.884), suggesting more consistent Phase\,1
service-routing decisions across repeated runs.
...
Gemini\,2.5\,Flash completes 59\,/\,60 entries with IWSR\,=\,93.3\,\%,
HR\,=\,5.0\,\%, and SS\,=\,0.884.
Its 4 IWSR failures are all Phase\,1 service misclassifications;
the Feedback Engine was not triggered in any case, confirming that the
correction loop addresses payload-level errors exclusively and cannot
recover from upstream service-routing mistakes.
...
\textbf{Limitations.}
GPT-4o-mini and Claude Haiku\,4.5 results include $k=5$ stability runs
($n=60$ entries each). Gemini\,2.5\,Flash completed 59\,/\,60 entries
(single run; $k=5$ not performed due to API quota constraints),
so SS\,=\,0.884 reflects a single-run estimate.
