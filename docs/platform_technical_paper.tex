...
Early intent-based networking (IBN) work demonstrated high-level intent
translation using deterministic rule-based systems. Rafiq et~al.~\cite{rafiq2020}
showed intent-driven VNF orchestration across multiple NFV platforms via
automatic descriptor generation, while Saraiva de~Sousa et~al.~\cite{saraiva2019}
provided a comprehensive taxonomy identifying AI integration as a key future
direction. Coronado et~al.~\cite{coronado2022} surveyed zero-touch network
management, establishing intent abstraction and AI-native closed loops as
foundational pillars for 6G automation. These works confirm the orchestration
automation paradigm but rely on deterministic rule-based translation without
probabilistic LLM reasoning.

\subsection{5G Network Exposure: CAPIF and CAMARA}

Ordonez-Lucena and Dsouza~\cite{camara2022} introduced the CAMARA initiative
for harmonised network API exposure, resolving operator-level fragmentation.
Molina~Sanchez et~al.~\cite{molina2022} implemented the CAPIF Core Function
as an open-source microservice aligned with 3GPP~TS~23.222, enabling standardised
API onboarding, discovery, and security. Fragkos et~al.~\cite{nefsim2022}
introduced NEFSim, an open experimentation platform for 3GPP NEF northbound
APIs. Turkovic et~al.~\cite{turkovic2025} demonstrated end-to-end integration
of CAMARA QoD and Location APIs in a real CAM testbed, measuring call-flow
latency of 230--340~ms. Li et~al.~\cite{li2025} extended exposure from core
networks to RAN-level capabilities via a proposed RAN Exposure Function.
Sheoran et~al.~\cite{sheoran2021} demonstrated that AI-driven placement and
scaling decisions outperform threshold-based autoscaling in microservice-based
5GC deployments. While these works establish robust standardised exposure
mechanisms, none incorporate intelligent intent translation.

\subsection{LLMs for Telecom and Structured API Output}

Maatouk et~al.~\cite{maatouk2025} provided one of the first structured analyses
of LLM deployment in telecom, highlighting hallucination risks and output
inconsistency. Lee et~al.~\cite{telagentbench2025} introduced TelAgentBench,
benchmarking 15 LLMs across reasoning, planning, tool use, and RAG in a
telecom service environment; thinking-enabled models consistently outperformed
open-source alternatives and multi-turn planning tasks significantly reduced
pass rates. Zeng et~al.~\cite{flowmind2023} demonstrated that API-grounded LLM
prompting achieves near-perfect workflow generation accuracy in the financial
domain, with user feedback further improving reliability. Liu et~al.~\cite{liu2024}
benchmarked LLM structured output compliance across 13 format types, finding
that even GPT-4 achieves an average score of only 0.40, confirming that format
compliance is a distinct and measurable challenge separate from semantic
correctness. Molner et~al.~\cite{aiora2025} proposed AIORA, an AI-native 6G
orchestration architecture integrating reinforcement learning, transformers,
and CAPIF-based API governance across edge-cloud continuum resources.

\subsection{Research Gap}

Existing work establishes robust exposure frameworks and confirms that LLMs
can generate structured API workflows under controlled conditions. However,
no prior work systematically evaluates LLM-based intent-to-API translation
\emph{within} CAPIF-compliant 5G service exposure environments under
telecom-specific constraints: schema conformity, semantic policy compliance,
hallucination characterisation, and multi-run output stability.
Table~\ref{tab:related} summarises the positioning of this work relative
...
\toprule
\textbf{Study} & \textbf{Telecom} & \textbf{LLM} & \textbf{Intent} &
\textbf{API Exec.} & \textbf{Reliability} & \textbf{CAPIF} \\
\midrule
FlowMind~\cite{flowmind2023}          & No  & Yes     & Yes       & Generic & Limited     & No  \\
Rafiq~\cite{rafiq2020}               & Yes & No      & Rule-based & NFV    & No          & No  \\
TelAgentBench~\cite{telagentbench2025} & Yes & Yes    & Agentic   & BSS     & Benchmark   & No  \\
Molina~\cite{molina2022}              & Yes & No      & No        & Yes     & No          & Yes \\
NEFSim~\cite{nefsim2022}              & Yes & No      & No        & Yes     & No          & No  \\
CAMARA~\cite{camara2022}              & Yes & No      & No        & Yes     & No          & Partial \\
Zero-Touch~\cite{coronado2022}        & Yes & Partial & Abstract  & No      & Conceptual  & No  \\
\textbf{This work}                    & Yes & Yes     & Yes       & Yes     & Empirical   & Yes \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=1.2em]
  \item \textbf{auto} — Attempts LLM-assisted planning if any LLM API key
        is configured (\texttt{GEMINI\_API\_KEY}, \texttt{OPENAI\_API\_KEY},
        or \texttt{ANTHROPIC\_API\_KEY}), falls back to deterministic if the
        LLM call fails or no key is set.
...
A process-wide thread-safe throttler enforces a configurable minimum
inter-call interval (default 4.5~s, yielding $\approx$13~RPM) for
Gemini free-tier accounts (15~RPM limit). HTTP~429 responses from any
provider fail immediately with a structured error, avoiding exponential
backoff hangs. The interval is tunable via \texttt{GEMINI\_MIN\_INTERVAL\_S}.

\subsection{Schema-Grounded API Context Engine (RAG)}

\texttt{backend/rag\_context.py} provides real-time schema injection into
LLM prompts. While termed RAG for consistency with the broader literature,
this mechanism uses deterministic YAML parsing rather than vector-based
semantic retrieval: schema injection is exact and fully reproducible across
runs, with no embedding model or similarity search involved. It parses CAMARA OpenAPI YAML files at runtime using PyYAML
and extracts:
...
RRF  (feedback recovery)   &    N/A  &    N/A\textsuperscript{a} \\
Avg.\ latency              & $<$1~ms & $\approx$3~s \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textsuperscript{a}RRF is undefined when no initially-invalid payloads exist (SCR = 100\%); the feedback loop was not triggered in any run.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item \textbf{Feedback engine was not exercised (RRF = N/A).}
        All 60 LLM-generated payloads passed schema validation on the first
        attempt (SCR = 100\%), leaving the feedback loop with no invalid
        payloads to recover from. RRF is therefore undefined (denominator
        = 0) and reported as N/A rather than 0\%, confirming that the
        RAG-based schema grounding is sufficient to produce valid payloads
        in a single pass.
...
\begin{itemize}[leftmargin=1.2em]
  ...
  \item OpenCAPIF stack (path configurable via \texttt{OPENCAPIF\_DIR} in \texttt{start.sh})
  \item At least one LLM API key: \texttt{GEMINI\_API\_KEY}, \texttt{OPENAI\_API\_KEY},
        or \texttt{ANTHROPIC\_API\_KEY} (for LLM-assisted mode)
\end{itemize}
...
\texttt{ANTHROPIC\_API\_KEY} & Anthropic API key (Claude Haiku, used for evaluation) \\
\texttt{ANTHROPIC\_MODEL} & Anthropic model (default: claude-haiku-4-5-20251001) \\
...
\begin{lstlisting}[language=json]
{
  ...
  "execution_results": [{ "status": "dry-run", ... }],
  "planner": {
...
The platform is accompanied by a suite of 65 unit tests (pytest) covering
the intent engine, three-layer validator, and executor, achieving full
pass rate. The platform source code, ground-truth dataset, and evaluation
...
Solutions for 5G and 6G Networks,'' \emph{IEEE Commun.\ Surveys Tuts.},
2022.
...
\bibitem{sheoran2021}
A.~Sheoran et al., ``AI-Driven Provisioning in the 5G Core,''
\emph{IEEE Internet Computing}, vol.~25, no.~2, pp.~18--26, 2021.

\bibitem{li2025}
Y.~Li, Y.~Gao, and D.~Li, ``SBA-Driven RAN Exposure Architecture for 6G
and Ecosystem Integration,'' \emph{IEEE/CIC ICCC Workshops}, 2025.
